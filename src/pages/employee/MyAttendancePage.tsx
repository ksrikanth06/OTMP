...
function DayCell({
  ...
  const isOTEligible = isPresent && (record.totalHours ?? 0) > 10;
  ...
  return (
    <div
      onClick={isPresent ? onClick : undefined}
      className={[
        'relative flex min-h-[76px] flex-col border-b border-r border-line p-1.5 transition',
        cellBg,
        isPresent ? 'cursor-pointer hover:bg-surface-overlay' : '',
      ].join(' ')}
    >
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            ...
            { label: 'Off Days',      value: summary.weekend, color: 'text-content-muted', accent: 'bg-line'    },
        ...
        <div className="rounded-card border border-line bg-surface-raised shadow-panel overflow-hidden">
          ...
          <div className="flex items-center gap-5 border-b border-line px-4 py-3">
            ...
            {[
              ...
              { dot: 'bg-line',     label: 'Weekend / Holiday' },
            ].map(({ dot, label }) => (
          ...
          >
            ...
            <div className="divide-y divide-line px-5 py-1">
              {([
                ...
                ['Hours Worked', <span key="hw" className={`font-semibold ${(selected.totalHours ?? 0) > 10 ? 'text-warning' : 'text-content-primary'}`}>{selected.totalHours} hrs</span>],
