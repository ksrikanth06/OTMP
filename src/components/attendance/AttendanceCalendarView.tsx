...
interface AttendanceCalendarViewProps {
  /** Employee whose attendance to load — the signed-in user, or one selected from a team tree. */
  empId: string;
}

export function AttendanceCalendarView({ empId }: AttendanceCalendarViewProps) {
  ...
  useEffect(() => {
    ...
    setRecords(null);
    setLoadError(false);
    getEmployeeAttendance(empId, year, month)
  ...
  return (
    <div className="space-y-6">
      ...
      {loadError && (
        <div className="rounded-card border border-dashed border-danger/30 bg-danger/5 py-16 text-center">
          <p className="text-sm text-danger">Unable to load attendance for this period.</p>
        </div>
      )}
